...
詳細度はクラスセレクタと同じ0,0,1,0以下になるように極力低くします。装飾的なスタイルや配置に関する指定は禁止します。
...
### 2. Object
プロジェクトを問わず、横断的に再利用できるようなコンポーネントを定義します。OOCSSにおける構造の役割を担当し、装飾的なスタイルを持つことはできません。
...
### 3. Parts
装飾的なスタイルを持つ、再利用できる小さなコンポーネントを定義します。目安としては、見出しやリスト、ボタンやラベルのような他のコンポーネントを内包できない小さなコンポーネントです。
...
### 4. Container
ContainerレイヤーではObjectレイヤーやPartsレイヤーを内包できる大きなコンポーネントを定義します。Containerコンポーネントがユーザーインターフェイスの見た目と構造を確定します。

内包するコンポーネントの配置を指定することができます。内包するコンポーネントの装飾を指定することも許容されます。
...
ただし、内包するコンポーネントに直接指定するのではなく、自身のElementに対して指定するほうが役割が明確になり、詳細度を低く保てます。		
...
### 6. Scope
Scopeレイヤーはカテゴリやブログのような特定の範囲でのスタイルを定義します。

Scopeレイヤー自身も内包するコンポーネントにも装飾的なスタイルや配置に関する指定をすることもできる、影響範囲の大きなレイヤーです。

Scopeレイヤーはコンポーネント単位ではなく、任意の範囲（スコープ）に対するスタイルを指定します。
...
### 7. Utility
Utilityレイヤーは汎用クラスを定義します。確実にスタイルを適応させるために`!important`を指定します。

他のレイヤーが持つよりも汎用的に使えたり、コードが冗長になってしまう場合に使います。
