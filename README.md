...
リセットをするよりも継承を利用するために、ルート要素の`font-size`はパーセンテージで指定、その他の要素やコンポーネントのサイズは`rem`と`em`で指定します。
...
内包するモジュールを上書きすることもできません。Layoutコンポーネントはレイアウトに特化したコンポーネントです。  
ただし、背景色のようにレイアウトによって影響範囲が変わるものは、最小限であればLayoutコンポーネントに含めることも許容します。
...
Layoutコンポーネントは常にルートを基準にサイズを決めるため、`rem`を基本単位とします。
...
### モジュールタイトル
モジュール化したファイルにはコンポーネントの名前や機能をコメントとして残します。スタイルガイドジェネレーター用のコメントを残してもいいでしょう。
